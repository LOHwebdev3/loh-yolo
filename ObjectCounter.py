...
class ObjectCounter(BaseSolution):
    ...
    def __init__(self, **kwargs):
        ...
        self.onAddCount = kwargs.get("onAddCount", None)

        self.show_in = self.CFG["show_in"]
        self.show_out = self.CFG["show_out"]

    def count_objects(self, current_centroid, track_id, prev_position, cls,time=0):
        """
            ...
            if line.intersects(self.LineString([prev_position, current_centroid])):
                ...
                if abs(self.region[0][0] - self.region[1][0]) < abs(self.region[0][1] - self.region[1][1]):
                    ...
                    if current_centroid[0] > prev_position[0]:  # Moving right
                        ...
                        if self.onAddCount:
                            self.onAddCount(self.names[cls], "IN", self.classwise_counts[self.names[cls]]["IN"],time)
                    else:  # Moving left
                        self.out_count += 1
                        self.classwise_counts[self.names[cls]]["OUT"] += 1
                        if self.onAddCount:
                            self.onAddCount(self.names[cls], "OUT", self.classwise_counts[self.names[cls]]["OUT"],time)
    ...
    def count(self, im0,time=0):
        ...
        for box, track_id, cls in zip(self.boxes, self.track_ids, self.clss):
            ...
            self.count_objects(current_centroid, track_id, prev_position, cls,time)  # Perform object counting
